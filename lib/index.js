...
export default function slackin({
  ...
  app.post('/invite', json(), (req, res, next) => {
    let chanIds;
    let permitted = channels;
    if (permitted) {
      let channels = req.body.channels;

      let notPermitted = channels.filter(c => !permitted.includes(c));
      if (notPermitted.length > 0) {
        return res
        .status(400)
        .json({ msg: `Not permitted channel(s): "${notPermitted.join(',')}"` });
      }

      channels = channels.map((c) => {
        return {name: c, id: slack.getChannelId(c)};
      });

      // Channel not found
      let notFound = channels.filter(c => !c.id).map(c => c.name);
      if (notFound.length > 0) {
        return res
        .status(400)
        .json({ msg: `Channel(s) not found "${notFound.join(', ')}"` });
      }

      // Extract the IDs
      chanIds = channels.map(c => c.id).concat(slack.getDefaultChannels());
    }
    ...
    invite({ token, org, email, channels: chanIds }, err => {
      if (err) {
        return res
        .status(400)
